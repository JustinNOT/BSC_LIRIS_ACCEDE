...
if __name__ == '__main__':
    ...
    for movieName in movieNames :
        ...
        for idx, movieFile in enumerate(moviesList):
            found = re.search('^' + movieName + '.mp4', movieFile.childNodes[0].nodeValue) # parse for line with trim values
            if (found):
        ...
        if(os.path.isfile(fileValence) and os.path.isfile(fileArousal)):
            ...
            meanDynamicContinuousArousal = meanDynamicContinuousArousal + max(continuousAnnotationsArousalValues) - min(continuousAnnotationsArousalValues)
            meanDynamicContinuousValence = meanDynamicContinuousValence + max(continuousAnnotationsValenceValues) - min(continuousAnnotationsValenceValues)
            ...
            pl.plot(np.asanyarray(continuousAnnotationsSeconds), np.asanyarray(continuousAnnotationsValenceValues), 'b', lw=1, label = 'mean')
            pl.fill_between(np.asanyarray(continuousAnnotationsSeconds), 
                            np.asanyarray(continuousAnnotationsValenceValues) - 1.96 * np.asanyarray(continuousAnnotationsValenceStd), 
                            np.asanyarray(continuousAnnotationsValenceValues) + 1.96 * np.asanyarray(continuousAnnotationsValenceStd),
            ...
            pl.plot(np.asanyarray(continuousAnnotationsSeconds), np.asanyarray(continuousAnnotationsArousalValues), 'b', lw=1, label = 'mean')
            pl.fill_between(np.asanyarray(continuousAnnotationsSeconds), 
                            np.asanyarray(continuousAnnotationsArousalValues) - 1.96 * np.asanyarray(continuousAnnotationsArousalStd), 
                            np.asanyarray(continuousAnnotationsArousalValues) + 1.96 * np.asanyarray(continuousAnnotationsArousalStd),
                            color='grey', alpha='0.3', label='95% confidence interval')
            pl.xlim([0, max(max(continuousAnnotationsSeconds), max(discreteAnnotationsEndSeconds))])
            pl.ylim([-1, 1])
            ...
            for i in range(nbGrid):
                ...
                if (nbGrid > 1):
                    ...
                    alphaValue = 1.0
                pl.plot(continuousAnnotationsArousalValues[firstIdx:lastIdx], continuousAnnotationsValenceValues[firstIdx:lastIdx], 'b-', alpha = alphaValue)
            
            pl.scatter(discreteAnnotationsArousalValues, discreteAnnotationsValenceValues, s=20, c='k', marker='o')
            
            pl.scatter([np.mean(discreteAnnotationsArousalValues)], [np.mean(discreteAnnotationsValenceValues)], s=50, c='r', marker='o')        
            pl.scatter([np.mean(continuousAnnotationsArousalValues)], [np.mean(continuousAnnotationsValenceValues)], s=50, c='c', marker='o')
            pl.xlim([-1,1])
            pl.xlabel('Arousal')
            pl.ylim([-1,1])
            pl.ylabel('Valence')
