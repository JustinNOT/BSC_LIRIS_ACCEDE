...
# DISCRETE (clip-level) VA model checkpoint
CKPT_PATH = ROOT / "checkpoints" / "va_resnet_gru_best.pth"
VIDEO_PATH = ROOT / "test" / "after_the_rain" / "After_The_Rain.mp4"
OUT_CSV = ROOT / "test" / "after_the_rain" / "After_The_Rain_discrete_va_1s.csv"


def load_checkpoint(device):
    print(f"[INFO] Loading discrete VA checkpoint: {CKPT_PATH}")
...
def build_model_from_checkpoint(obj, device):
    ...
    model.to(device)
    model.eval()
    print("[INFO] Discrete VA model built and weights loaded.")
    return model
...
def main():
    print("[INFO] dump_discrete_va_1s_after_the_rain.py starting")
    ...
    with torch.no_grad():
        for idx, t in enumerate(times_1s):
            ...
            out_np = out.squeeze(0).cpu().numpy().reshape(-1)
            if out_np.shape[0] != 2:
                print(f"[WARN] Expected 2 outputs (V/A), got shape {out_np.shape}; skipping.")
                continue

            pred_valence = float(out_np[0])
            pred_arousal = float(out_np[1])

            rows.append([float(t), pred_valence, pred_arousal])
    ...
    with OUT_CSV.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["time_sec", "pred_valence", "pred_arousal"])
